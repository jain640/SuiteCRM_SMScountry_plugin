...
function smsad_sms_templateController(action) {
	...
	var save = function(update) {
		...
		window.parent.$('.ui-dialog-content:visible').dialog('close');
		var func = smsad_sms_templateCopyId || $('input[name="update_exists_template"]').prop('checked') ? 'update' : 'createCopy';
		$('#template_messages').html(SUGAR.language.translate('Campaigns', 'LBL_TEMPLATE_SAVING'));
		$.post('index.php?entryPoint=smsad_sms_templateData&func=wizardUpdate&rand=' + Math.random(), {'func': func, 'smsad_sms_templateId': smsad_sms_templateCopyId ? smsad_sms_templateCopyId : $('#template_id').val(), 'body_html': $('#email_template_editor').getMozaikValue(), 'name': $('#template_name').val(), 'subject': $('#template_subject').val(), }, function(resp) {
			resp = JSON.parse(resp);
			if (resp.error) {
				console.error(resp.error);
			}
			else {
				if (!smsad_sms_templateCopyId && func == 'createCopy') {
					smsad_sms_templateCopyId = resp.data.id;
	...
	var create = function() {
		...
		var func = smsad_sms_templateCopyId || $('input[name="update_exists_template"]').prop('checked') ? 'update' : 'createCopy';
		$('#template_messages').html(SUGAR.language.translate('Campaigns', 'LBL_TEMPLATE_SAVING'));
		$.post('index.php?entryPoint=smsad_sms_templateData&rand=' + Math.random(), {'func': func, 'smsad_sms_templateId': smsad_sms_templateCopyId ? smsad_sms_templateCopyId : $('#template_id').val(), 'body_html': $('#email_template_editor').getMozaikValue(), 'name': $('#template_name').val(), 'subject': $('#template_subject').val(), }, function(resp) {
			resp = JSON.parse(resp);
			if (resp.error) {
				console.error(resp.error);
			}
	...
	console.log('smsad_sms_templateController()', action)
}
function EmailTrackerController(action, campaignId) {
	...
	var create = function() {
		...
		window.parent.$('.ui-dialog-content:visible').dialog('close');
		$.post('index.php?entryPoint=campaignTrackerSave', {module: 'CampaignTrackers', record: '', action: 'Save', campaign_id: _campaignId, tracker_name: trackerName, tracker_url: trackerURL, is_optout: $('#is_optout').prop('checked') ? 'on' : '', response_json: true}, function(resp) {
			resp = JSON.parse(resp);
			if (resp.data.id) {
	...
	var save = function() {
		...
		hideFieldErrorMessages();
		window.parent.$('.ui-dialog-content:visible').dialog('close');
		$.post('index.php?entryPoint=campaignTrackerSave', {module: 'CampaignTrackers', record: $('select[name="tracker_url"] option:selected').attr('data-id'), action: 'Save', campaign_id: _campaignId, tracker_name: trackerName, tracker_url: trackerURL, is_optout: $('#is_optout').prop('checked') ? 'on' : '', response_json: true}, function(resp) {
			resp = JSON.parse(resp);
			if (resp.data.id) {

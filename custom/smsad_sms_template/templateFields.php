...
{
    global $app_list_strings, $beanList, $beanFiles;
    $allowedBeans = array('Accounts','Contacts','Leads');
    ...
    foreach ($app_list_strings['moduleList'] as $key => $name) {
        if (in_array($key,$allowedBeans) && isset($beanList[$key]) && isset($beanFiles[$beanList[$key]]) && !str_begin($key, 'AOW_') && !str_begin($key, 'zr2_')) {

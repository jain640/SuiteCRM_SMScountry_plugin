...
$GLOBALS['log']->info("smsad_sms_template detail view");

if ($has_campaign || $inboundEmail) {
    $xtpl = new XTemplate ('custom/modules/smsad_sms_template/EditView.html');
} else {
    $xtpl = new XTemplate ('custom/modules/smsad_sms_template/EditViewMain.html');
} // else
...
require_once("custom/modules/smsad_sms_template/templateFields.php");
...
//    $xtpl->assign("INSERT_VARIABLE_ONCLICK", "insert_variable(document.EditView.variable_text.value)");
    $xtpl->assign("INSERT_VARIABLE_ONCLICK", "insert_variable_text(document.EditView.description, document.EditView.variable_text.value)");
	...
	//status
	$templateStatus = !empty($focus->status_id) ? $focus->status_id : '';
	if (empty($_REQUEST['record'])) {
		// new record, default to campaign
		$xtpl->assign("STATUSDROPDOWN", get_select_options_with_id($app_list_strings['document_status_dom'], 'Active'));
	} else {
		$xtpl->assign("STATUSDROPDOWN", get_select_options_with_id($app_list_strings['document_status_dom'], $templateStatus));
	}

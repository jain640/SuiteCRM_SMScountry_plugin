...
class smsad_sms_templateFormBase {
...
EOF;
	...
	$javascript->setSugarBean(new smsad_sms_template());
...
$the_form .= <<<EOQ
				...
				<form name="${prefix}smsad_sms_templateSave" onSubmit="return check_form('${prefix}smsad_sms_templateSave')" method="POST" action="index.php">
					<input type="hidden" name="${prefix}module" value="smsad_sms_template">
					<input type="hidden" name="${prefix}action" value="Save">
EOQ;
		$the_form .= $this->getFormBody($prefix, $mod, "${prefix}smsad_sms_templateSave", "20");
$the_form .= <<<EOQ
...
EOQ;
	...
	public function processImages(&$focus, $useSiteURL, $entryPoint, $useUploadFolder) {
		...
		$smsad_sms_templateBodyHtml = from_html($focus->body_html);
		if (strpos($smsad_sms_templateBodyHtml, '"cache/images/')) {
			$matches = array();
			preg_match_all('#<img[^>]*[\s]+src[^=]*=[\s]*["\']cache/images/(.+?)["\']#si', $smsad_sms_templateBodyHtml, $matches);
			foreach ($matches[1] as $match) {
				$filename = urldecode($match);
				if ($filename != pathinfo($filename, PATHINFO_BASENAME)) {
					// don't allow paths there
					$smsad_sms_templateBodyHtml = str_replace("cache/images/$match", "", $smsad_sms_templateBodyHtml);
					continue;
				}
				...
				if (file_exists($file_location)) {
					...
					if (!copy($file_location, $newFileLocation)) {
						...
						$smsad_sms_templateBodyHtml = str_replace("cache/images/$match", $secureLink, $smsad_sms_templateBodyHtml);
		...
		if($preProcessedImages) {
			$focus->body_html = $smsad_sms_templateBodyHtml;
		}
		...
		foreach($focus->attachments as $note)
		{
			if( !empty($note->id) && $note->new_with_id === FALSE)
			{
				...
				else
				{
					...
					//Need to do this so that attachments show under an smsad_sms_template correctly for a normal user
					global $current_user;
	...
	///////////////////////////////////////////////////////////////////////////////
		...
		$GLOBALS['log']->debug("Saved record with id of ".$return_id);
			handleRedirect($return_id, "smsad_sms_template");
		}else{
			return $focus;
		}
	}

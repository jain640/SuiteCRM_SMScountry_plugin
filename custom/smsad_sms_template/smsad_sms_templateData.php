...
$smsad_sms_templateId = isset($_REQUEST['smsad_sms_templateId']) && $_REQUEST['smsad_sms_templateId'] ? $_REQUEST['smsad_sms_templateId'] : null;
if(isset($_REQUEST['campaignId'])) {
    $_SESSION['campaignWizard'][$_REQUEST['campaignId']]['defaultSelectedTemplateId'] = $smsad_sms_templateId;
}

if(preg_match('/^[a-f0-9]{8}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{4}-[a-f0-9]{12}$/', $smsad_sms_templateId) || !$smsad_sms_templateId) {

    $func = isset($_REQUEST['func']) ? $_REQUEST['func'] : null;

    $fields = array('document_name',  'status_id',  'category_id',  'subcategory_id', 'body_html', 'subject', 'name');

    // TODO: validate for email template before save it!

    include_once 'custom/modules/smsad_sms_template/smsad_sms_templateFormBase.php';

    switch($func) {

        case 'update':
            $bean = BeanFactory::getBean('smsad_sms_template', $smsad_sms_templateId);
            foreach($bean as $key => $value) {
                if(in_array($key, $fields)) {
                    $bean->$key = $_POST[$key];
                }
            }
            $formBase = new smsad_sms_templateFormBase();
            ...
            if(!empty($_REQUEST['campaignId']) && !empty($_SESSION['campaignWizard'][$_REQUEST['campaignId']]['defaultSelectedMarketingId'])) {
                ...
                if(in_array($marketingId, $marketings)) {
                    ...
                    $marketing->template_id = $smsad_sms_templateId;
                    $marketing->save();
                }
                else {
        ...
        case 'createCopy':
            $bean = BeanFactory::getBean('smsad_sms_template', $smsad_sms_templateId);
            $newBean = new smsad_sms_template();
        ...
        case 'uploadAttachments':
            $formBase = new smsad_sms_templateFormBase();
            $focus = BeanFactory::getBean('smsad_sms_template', $_REQUEST['attach_to_template_id']);
        ...
        default: case 'get':
            if($bean = BeanFactory::getBean('smsad_sms_template', $smsad_sms_templateId)) {
                $fields = array('id', 'name',  'document_name',  'status_id',  'category_id',  'subcategory_id', 'body', 'body_html', 'subject');
                foreach ($bean as $key => $value) {
                    if (in_array($key, $fields)) {
                        $data[$key] = $bean->$key;
                    }

 ...
 ********************************************************************************/
// smsad_sms_template is used to store email email_template information.
require_once('modules/smsad_sms_template/smsad_sms_template_sugar.php');
class smsad_sms_template extends smsad_sms_template_sugar {
    ...
    var $from_name;
    var $from_address;
    var $table_name = "smsad_sms_template";
    var $object_name = "smsad_sms_template";
    var $module_dir = "smsad_sms_template";
    var $new_schema = true;
     ...
     */
    public function smsad_sms_template(){
...
//function all string that match the pattern {.} , also catches the list of found strings.
    ...
    function parse_template_bean($string, $bean_name, &$focus)
    {
        ...
        foreach ($lead->field_defs as $field_def) {
            ...
            $repl_arr = smsad_sms_template::add_replacement($repl_arr, $field_def, array(
        ...
        if ($bean_name == 'Contacts') {
            ...
            if (!empty($focus->assigned_user_id)) {
                $user = new User();
                $user->retrieve($focus->assigned_user_id);
                $repl_arr = smsad_sms_template::_parseUserValues($repl_arr, $user);
            }
             ...
             */
            $repl_arr = smsad_sms_template::_parseUserValues($repl_arr, $current_user);
        } else {
    ...
    function parse_template($string, &$bean_arr)
    {
        foreach ($bean_arr as $bean_name => $bean_id) {
            ...
            if (isset($this) && isset($this->module_dir) && $this->module_dir == 'smsad_sms_template') {
                $string = $this->parse_template_bean($string, $bean_name, $focus);
            } else {
                $string = smsad_sms_template::parse_template_bean($string, $bean_name, $focus);
            }
        }
        return $string;
    }
    ...
    {
        $template = new smsad_sms_template();
